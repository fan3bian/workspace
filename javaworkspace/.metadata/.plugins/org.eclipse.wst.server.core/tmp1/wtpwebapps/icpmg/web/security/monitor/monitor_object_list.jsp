...
function statusformater(value, row, index) {
	...
	//监控配置按钮
	function monitorOperate(value, row, index) {
	
		if (!value) {
			return "";
		}

		var str = "<div style=\"height:30px;overflow:hidden;\"><a href=\"javascript:void(0);\" onclick=\"monitorConfigInit(\'"	+ row.cuserid
				+ "\', \'" + row.securityid + "\', \'"+ row.manip + "\', \'" + row.objectid
				+ "\');\" class=\"j-guanli\" iconCls=\"icon-config\" plain=\"true\" title=\"监控配置\"></a>";

		return str;
	}
	
	//打开监控配置主页面（SNMP、监控配置、日志配置）
	function monitorConfigInit(cuserid,securityid,manip,objectid){
		$('#centerTab').panel({
			href:"${pageContext.request.contextPath}/security/monitorConfigInit.do" ,
		...
		<table title="" style="width:100%;"  id="fw_object_grid">
			<thead>
				<tr>
					...
					<th data-options="field:'shopid',width:60,align:'center',formatter:monitorOperate">操作</th>
				</tr>
			</thead>
		</table>
		</div>
