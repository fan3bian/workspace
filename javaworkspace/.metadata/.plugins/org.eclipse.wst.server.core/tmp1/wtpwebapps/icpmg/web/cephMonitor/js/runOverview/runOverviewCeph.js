...
//	var neid = "于颜华-small";
	var neid = objectid ;
	var ceph_status_action = context_path + "/ceph_runoverview/getcephRunStatus.do";
	var ceph_alarmnum_action = context_path + "/ceph_runoverview/getcephRunAlarm.do";
	var ceph_useage_action = context_path + "/ceph_runoverview/getcephUseage.do";
	var ceph_index_action = context_path + "/ceph_runoverview/getcephIndexData.do";
	var ceph_disk_action = context_path + "/ceph_runoverview/getcephDiskData.do";
	var ceph_disk_mount_action = context_path + "/ceph_runoverview/getcephDiskSize.do";

/********************************************************************************************
 *跳转至详情 
 */
	
	$('#ceph-more').click(function(){
		pass_neid = neid;
		$('#centerTab').panel({
			href:'/icpmg/web/cephMonitor/jsp/cephDetails.jsp',
			queryParams:{
				neid:pass_neid
			 }
			});	
 ...
 */
	...
	function setStatusMessage(healthStatus,feeStatus,nename,ipaddr)
	{
		if(healthStatus == "1")
		{
			$("#ceph-health").css('background-position','0 0');
		}	

		$("#ceph-nename").empty();
		$("#ceph-nename").append("云服务器名称 : &nbsp;&nbsp;&nbsp" + neid);
		$("#ceph-ipaddr").empty();
		$("#ceph-ipaddr").append("云服务器IP地址 : " + ipaddr);
	}	
	...
	function setStatusTime(day,hour,minute)
	{
		...
		$("#ceph-day").append(day);
		$("#ceph-hour").empty();
		$("#ceph-hour").append(hour);
		$("#ceph-min").empty();
		$("#ceph-min").append(minute);
	}
 ...
 */	
	...
	({
		...
		async:true, 
		data:{neid:neid},
		url:ceph_alarmnum_action,
		dataType:'json',
		success:
			function(ret)
			{
				$("#ceph-alarm-num").empty();
				$("#ceph-alarm-num").append('&nbsp;' + ret + '&nbsp;');
			}
	});
 ...
 */	
	...
	var cpuGaugeChart = echarts.init(document.getElementById('ceph-cpu-zone'));
	var cpuGaugeOption = {
			title : {
		        text: '云服务器CPU利用率',
		        x:'left',
 ...
 * 内存利用率图表
 */	
	var memGaugeChart = echarts.init(document.getElementById('ceph-mem-zone'));
 ...
 */	
    ...
    var indexChart = echarts.init(document.getElementById('ceph-index-zone'));
 ...
 */		
	...
	var memIndexFlag = '0';
//	var diskIndexFlag = '0';
	$('#ceph-time-cobbox-out').hide('slow');
		...
		({
			...
			url:ceph_index_action,
			dataType:'json',
			success:
				function(ret)
				{
				$('#ceph-index-list-zone .index-menu .index-list .index-dropdown ').show('slow');
				$('#ceph-time-cobbox-out').show('slow');
...
//					diskIndexData=ret.diskList;
					...
					$('#cpu-box').val('1');
					indexChart = echarts.init(document.getElementById('ceph-index-zone'));
 ...
 */
    ...
    var diskDetailChart = echarts.init(document.getElementById('ceph-single-disk-zone'));
 ...
 */
	...
	var diskSizeChart = echarts.init(document.getElementById('ceph-disk-mount-zone'));
