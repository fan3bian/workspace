...
function loadDataGrid(){
	$('#hostlist_datagrid').datagrid({
	    ...
	    url:'${pageContext.request.contextPath}/rescpool/getHLWHosts.do?poolid_=10001', 
...
/* function hostingOperMonitor(hostid){
	$.ajax({
		url:"${pageContext.request.contextPath}/instance/hostingOperMonitorDialog.do?",
		data:{
			neid_: hostid
		},
		type : "post",
		...
		success:function(result){
			...
			$('#hosting_monitor_dialog').html(result);
			$('#hosting_monitor_dialog').dialog('open');
		}
	});
} */
//操作
/* function opsFormater(value,rowData,index){
	var str='';
	str += '<a onclick="hostingOperMonitor(\''+rowData.servername+'\');">运行监控</a>';
	return str;
} */
...
function kpi_dataAjax(){
	...
	$.ajax({
		...
		data : {'poolid':'10001', 'cpu_total_kpi':'HOST_CPU_TOTAL', 'cpu_used_kpi':'HOST_CPU_USAGE',
...
<div class="continnerWrapper">
	...
	<div style="margin-left:20px;margin-top:5px; padding:5px;">
		<span style="font: bold 14px tahoma, arial, helvetica, sans-serif;">互联网资源池主机数量：</span>
