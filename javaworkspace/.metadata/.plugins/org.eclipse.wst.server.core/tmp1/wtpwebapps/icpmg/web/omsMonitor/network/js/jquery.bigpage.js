		...
		this.ajaxpage = function(param){
			...
			this.applyBuildTable = function(){
				...
				if (data != null && $.isArray(data)) {
					...
					//debugger;
					/*$.post(
							this.config.ajaxData.url,
							this.config.ajaxData.params,
							function(results){
						alert(results);
	...
	$.bigPage.addCssWidget({
		...
		format :function($table){
			...
			for(var i=beginOption;i<=endOption;i++){
				if(currentOption == i){
					as += "<span style='float:left' class='current'>" + i + "</span>";
				}else{
					as += "<a style='float:left' href='javascript:void(0)' ajaxpage='skip' pageNumber=" + i + "  >" + i + "</a>";
				}
			}				
			var footPageHtml = '<div ajaxpage="foot" class="bigpage" style="float:left" ><a style="float:left" class="' + prevClass +'" href="javascript:void(0)" ajaxpage="prev" >上一页</a>' + as + '<a style="float:left" class="' + nextClass + '" href="javascript:void(0)" ajaxpage="next"  >下一页</a><span style="float:left">共&nbsp;</span><span ajaxpage="count" style="color: red;float:left" >0</span><span style="float:left">&nbsp;页 跳转到</span> <input type="text" size="5" maxlength="5" ajaxpage="text" style="float:left"> <span style="float:left">页</span>  <a href="javascript:void(0)" ajaxpage="skipA" style="float:right">GO</a></div>';
