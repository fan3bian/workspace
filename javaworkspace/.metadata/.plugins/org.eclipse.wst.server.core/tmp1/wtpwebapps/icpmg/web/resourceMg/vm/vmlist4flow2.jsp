...
//	value ="";
	if((null == value || "" == value) && row.isimage == '1'){
		...
		return "<a href=\"javascript:void(0);\" onclick=\"createDialog(\'" + row.cpunum + "\', \'" + row.memnum + "\', \'" + row.disknum + "\', \'" + row.detailid + "\', \'" + row.cuserid + "\', \'" + row.diskDisplayStr+ "\');\">创建</a>";
	}else if('2' == value){
...
//		return "创建成功";
        return "<a href=\"javascript:void(0);\" onclick=\"netHandler(\'" + row.imode + "\', \'" + row.uuid + "\', \'" + row.interbw + "\', \'" + row.interport + "\', \'" + row.detailid + "\', \'" + row.cuserid+ "\', \'" + row.flowid + "\', \'" + row.detailid  + "\');\">网络配置</a>";
	}else if('5' == value){
		return "请稍候";
	}else if('-1' == value){
	    return "<a href=\"javascript:void(0);\" onclick=\"createDialog(\'" + row.cpunum + "\', \'" + row.memnum + "\', \'" + row.disknum + "\', \'" + row.detailid + "\', \'" + row.cuserid + "\', \'" + row.diskDisplayStr+ "\');\">创建失败,重现创建</a>";
	}else if('-2' == value){
...
function netHandler(imode, serverid, interbw, interport, detailid,serveruserid,flowid,_detailid){
...
//	getNetinfoPre(imode,serverid,interbw,detailid,serveruserid,_detailid)
...
//			$('#mpass').textbox('setValue', result.password).textbox('setText', result.password);
			...
			if(($vpnaddr!="" && $vpnaddr!=null)&&($vpnuser!=""&&$vpnuser!=null)){
			alert(($vpnaddr!=""||$vpnaddr!=null));alert($vpnuser);
		    $('#vpnaddr').textbox('setValue', $vpnaddr).textbox('setText',$vpnaddr);
			var $vpnusers = $vpnuser.split(';')
		    $("#data_table1 tr:gt(5)").remove(); 
...
function getNetinfoVM(imode, serverid, interbw, interport, detailid,serveruserid,flowid,detailid){
	jQuery.ajax( {
		url : "${pageContext.request.contextPath}/jsonvm/getVmIntraip.do",
		data : {'serverid' : serverid},
		type : "post",
...
function getNetinfoPre(imode, serverid, interbw, interport, detailid,serveruserid,_flowid,_detailid){
	jQuery.ajax( {
		url : "${pageContext.request.contextPath}/vm/getPreNetInfo.do",
		data: {
			'serverid': serverid,
...
function createDialog(cpunum, memnum, disknum, detailid,serveruserid,diskDisplayStr){
	$('#vmCreateForm').form('clear');
	$('#cpunum').textbox('setValue', cpunum).textbox('setText', cpunum);
	$('#memnum').textbox('setValue', memnum).textbox('setText', memnum);
	$('#disknumDisplay').textbox('setValue', diskDisplayStr).textbox('setText',diskDisplayStr);
	$('#disknum').textbox('setValue', diskformater(disknum)).textbox('setText', diskformater(disknum));
	$('#detailid_').val(detailid);
	$('#serveruserid_').val(serveruserid);
	platformCombobox();
	$('#w').window('open');
	...
	if($('#vmCreateForm1').form('validate')){
		$('#vmCreateForm1').form('submit', {
					...
					onSubmit : function(param) {
						...
						if($("#data_table1 tr").length>5){
						param.vpnaddr = $('#vpnaddr').textbox('getValue');
						var $vpnusers="";
						if(param.vpnaddr!="" && param.vpnaddr!=null){
						var $sizeofvpn = $("#data_table1 tr:gt(5)").length;
						for(var i=0;i<$sizeofvpn;i++){
						   
						   $vpnusers+= $("#vpnuser"+(i+1)).val()+"-"+$("#vpnpasswd"+(i+1)).val()+";"
			
						}
						}
						param.vpnuser = $vpnusers;
				}
		...
		<table title="" style="width:100%;"  id="vmlist_grid">
			<thead>
				<tr>
					...
					<th data-options="field:'disknum',width:90,align:'center',formatter:diskformater,hidden:'true'">硬盘(G)</th>
					<th data-options="field:'diskDisplayStr',width:90,align:'center'">硬盘(G)</th>
			...
			<table id="data_table" align="center" style="width:100%">
				...
				<tr>
					...
					<td class="FieldInput2">
						<input id="disknumDisplay" class="easyui-textbox" style="height:30px;" data-options="editable:false"/>
						<input type="hidden" id="disknum" class="easyui-textbox" style="height:30px;" data-options="editable:false"/>
					</td>
				</tr>
				<tr>
