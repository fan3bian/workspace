...
var fwtoolbar = [
				/* {
				...
				}, */
                 {
					...
					iconCls:'icon-add',
				    handler:function(){ 
				    	resetSecurityBasicConfigEW();
				    	resetSecurityBasicConfig();
				    	resetSecurityNetworkConfig();
				        $("#securityServiceOpen_typeConfig").window('open');
				    }
				},
				
				 {
					...
					iconCls:'icon-delete',
				    handler:function(){ 
				    	var row = $('#fw_object_grid').datagrid('getData').rows[$("#rowid").val()];
						if($("#rowid").val()=='-1'){
							$.messager.alert('提示信息','请选择一个实例！', 'info');
							return;
						}
						var objectid = row.objectid;
						var protype = row.protype;
						if(protype=='1'){
							$.messager.alert('提示信息','请选择一个南北向实例！', 'info');
							return;
						}
						$('#conditionForm').form('submit', {    
						    url:"${pageContext.request.contextPath}/security/securityDestroySN.do",    
		 ...
		 <input type="hidden" id="rowid" name="rowid">
			<thead>
				<tr>
					...
					<th data-options="field:'objectid',width:70,align:'center'">编码</th>
...
<div hidden>
<jsp:include page="securityServiceOpen_typeConfig.jsp"></jsp:include>
...
<jsp:include page="securityServiceOpen_basicConfigEW.jsp"></jsp:include>
<jsp:include page="securityServiceOpen_successEW.jsp"></jsp:include>
</div>
