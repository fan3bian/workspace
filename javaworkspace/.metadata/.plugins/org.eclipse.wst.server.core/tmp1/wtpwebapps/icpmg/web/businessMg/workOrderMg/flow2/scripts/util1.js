...
//加载选择CPU
function SelCpu(){
	var obj_5=$('.product-form-cell').eq(8).find('.item-01').children('li');
...
function rollStrip_1(value){
	var vals=null;//最后的百分比
	var strip=$('.product-form-cell .uc-slider1 .strip');//动画动像
	var chunk=$('.product-form-cell .uc-slider1 .strip .chunk');
	var leftValue=12;
	var width1 = 229;
    var width2 = 139;
	var width3 = 139;
	...
	function count(num){
        ...
        if(num<=368 && num>229){
        	//alert(num);
        	vals=600+Math.round((num-229)/(368-229)*(1500-600));
        	//alert(vals);
        ...
        if(num>368 && num<=508){
     		vals=1500+Math.round((num-368)/(508-368)*(4000-1500));
     		vals=Math.round(vals/100)*100;//取整，100的倍数
     		
     		if(vals>1500&&vals<=1800)
        		vals=1800;
        	else if(vals>1800&&vals<=2000)
        		vals=2000;
        	else if(vals>2000&&vals<=2500)
        		vals=2500;
     		else if(vals>2500&&vals<=3000)
        		vals=3000;
     		else if(vals>3000&&vals<=4000)
        		vals=4000;
        ...
        $('.product-form-cell .num1 input').val(vals);
        //$('#intrabw').val(vals);
	}
	...
	//})
	//$('.product-form-cell .num1 input').keyup(function(){
		//var val=$(this).val();
	    var val=value;
		$('.product-form-cell .uc-slider1 .strip .val').fadeIn(500);
		$('.product-form-cell .uc-slider1 .strip .val').text($('.product-form-cell .num1 input').val()+"G");
		if(val<=600){
			...
			left = (val-600)/(1500-600)*width2+width1;
			strip.css('width',left+'px')
		} else if(val>1500 && val<=4000){
			left = (val-1500)/(4000-1500)*width3+width2+width1;
			strip.css('width',left+'px')	
		}
